...
@Model class Currency: Codable {
    ...
    init(currency_id: UUID = UUID(),currency_name: String = "", currency_symbol: String = "", currency_alphabetic_code: String = "", currency_entity: String = "", currency_numeric_code: String = "", currency_minor_unit: Int = 2) {
    ...
    func description() -> String {
        return "Currency [ID: \(currency_id), Name: \(currency_name), Symbol: \(currency_symbol), Short Name: \(currency_alphabetic_code), Entity: \(currency_entity), Numeric Code: \(currency_numeric_code), Minor Unit: \(currency_minor_unit)]"
    }
    ...
    required init(from decoder: Decoder) throws {
        ...
        currency_id = try values.decode(UUID.self, forKey: .currency_id)
